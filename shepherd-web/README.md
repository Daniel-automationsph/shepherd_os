...
- **Map**: `react-leaflet` + OpenStreetMap raster tiles, real barangay names/population,
  **placeholder lat/lng** — see the comment block in `src/data/mockData.js`. Replace with
  real coordinates before using this for actual outreach planning. OSM's free tile server
  is fine for development, not for production traffic at scale — swap the `TileLayer`
  `url` in `BarangayMap.jsx` for a paid provider (Mapbox, MapTiler, Stadia Maps) before
  real production use.
